...
def main():
    ...
    if not cap.isOpened():
        ...
        text_to_speech("Error: Could not open video capture")
        return

    print("Press 'q' to quit")
    text_to_speech("Press q to quit")

    while True:
        ret, frame = cap.read()
        if not ret:
            print("Failed to grab frame")
            text_to_speech("Failed to grab frame")
            break
        ...
        try:
            ...
            if ocr_data is not None:
                ...
                text_to_speech("Error during OCR processing")
        except Exception as e:
            print(f"Error during OCR: {e}")
            text_to_speech(f"Error during OCR: {e}")
